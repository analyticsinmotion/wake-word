...
wake-word/
  src/
    ...
    wakeWordCore.ts           # Pure logic shared by the host and the engine: protocol, handoff order, route availability, phrase checks, diagnostics, session stats
...
set-aside route contributes no keyword lines, and what is logged and notified,
and when. `tests/unit/extensionAvailability.test.ts` drives `activate()` with
