...
wake-word/
  src/
    ...
    sherpaEngine.ts           # SherpaEngine: the engine child process (the native binary in bin/, or audio-engine.js under system Node.js), every platform; model download
  ...
  engine-rs/           # Rust implementation of the engine child process, built into bin/. The engine the extension runs; see below
    ...
    scripts/stage.mjs           # Puts the binary, ONNX Runtime, the Silero model, on Windows the Visual C++ runtime libraries, and their notices into bin/; signs on macOS
    notices/VC-RUNTIME-NOTICES.md  # Ships in bin/ on Windows with the Visual C++ runtime libraries
...
`extension.ts` owns all VS Code API interactions. `sherpaEngine.ts` implements `ISpeechEngine` on every platform. The engine is a child process, never code in the extension host: the native binary in `bin/`, or `audio-engine.js`, which runs under system Node.js (not Electron) so native audio addons load correctly. Keep this separation clean.
...
**SherpaEngine** spawns the engine child process, and `ENGINE_KIND` in `sherpaEngine.ts` decides which: `"native"`, the value it has, is `bin/wake-word-engine` (`.exe` on Windows) from the installed extension, with no arguments, and `"node"` is `engine/audio-engine.js` under system Node.js. Both are packaged, take the same config line and speak the same protocol, so changing that one constant is the whole of switching between them; `engineLaunch()` turns the kind into a command and arguments, and the constructor's last parameter overrides the constant, which is how the tests drive both. Before it spawns the native binary, `prepareNativeEngine()` checks that the file exists and, on macOS and Linux, that it is executable, setting mode 755 when it is not: the package records the mode and the editor restores it on install, but an extension unpacked another way can lose it. A binary that is missing, cannot be made executable, or fails to spawn is reported with its path, never as a missing Node.js. The binary needs nothing installed: it finds ONNX Runtime and the voice activity model beside itself (see the Rust engine below). `findSystemNode()`, `wakeWord.nodePath`, `NODE_NOT_FOUND_MESSAGE` and the Node.js line in the diagnostics serve the `"node"` kind and are kept for it; with the native engine the setting changes nothing but still rebuilds the engine.

The rest of this paragraph describes the `"node"` kind. The child uses `decibri` (5.7.0) for mic capture and `sherpa-onnx` for keyword spotting. Config is sent as a JSON line to stdin. System Node.js is required because Electron cannot load native addons at the correct ABI, and Node.js 22 or later is its requirement on every platform; a spawn that fails with ENOENT reports `NODE_NOT_FOUND_MESSAGE`, which says so and links to nodejs.org, because Windows needed no Node.js before 0.13.0. `findSystemNode()` caches the executable it finds for the session, because the lookup spawns `where node` or `which node` synchronously on the extension host thread. The `wakeWord.nodePath` override and the bare `node` fallback are never cached, and a spawn that fails with ENOENT calls `clearNodePathCache()` so the next start looks again. The child and the lookup are both started with `windowsHide: true`, so Windows gives neither console program a window. Model extraction starts no process at all (see below).
...
20. In the Extension Development Host, check the output channel after a start, a detection, and a resume for every timing line: `bpe-load`, `tokenise`, `model-load`, `mic-open`, `start-to-ready`, `detect-to-release`, `detect-to-command`, `pause-to-ack`, `resume-mic-open`, and `resume-to-ready`. `bpe-load` and `tokenise` are the host's and come before the `Spawning:` line, which names `bin/wake-word-engine`. With `ENGINE_KIND` set to `"node"` there is also `modules-load`, and the engine's own `bpe-load` and `tokenise` after the `Spawning:` line. Note the start and resume figures.
...
25. Run **Wake Word: Show Diagnostics**. The output channel has the report from "=== Wake Word Diagnostics ===" to "=== End Diagnostics ===", with an `Engine binary:` line that ends in `self-test OK` and names `ort=` and `vad-model=` files in the extension's `bin/`, the Node.js version, the model marked downloaded, and `~` in place of your home directory. Choose **Copy to Clipboard** and paste: the same lines. Run it again and choose **Show Log**.
...
30. Set `wakeWord.nodePath` to a path that does not exist. The engine restarts and listens as before, because the native engine uses no Node.js. With `ENGINE_KIND` set to `"node"`, the error notification says Wake Word requires Node.js 22 or later on all platforms and names `wakeWord.nodePath`.
...
35. Download a platform `.vsix` from a CI run and install it with **Extensions: Install from VSIX...**. In the installed extension's folder, `bin/` holds `wake-word-engine` (executable on macOS and Linux), the ONNX Runtime library, `silero_vad.onnx`, and the two notices, plus on Windows the four Visual C++ runtime libraries and `VC-RUNTIME-NOTICES.md`, and `bin/wake-word-engine --self-test` prints `SELF-TEST:OK` with `ort=` and `vad-model=` naming files in that `bin/`. Do this on Windows, on macOS, and on a Linux older than the runner (for example Ubuntu 22.04 or a RHEL 8 derivative).
36. On a machine with no Node.js on the PATH, install a platform `.vsix` and enable listening: the `Spawning:` line names `bin/wake-word-engine`, phrases are detected, and pause and resume work. On Windows, do this on a machine without the Microsoft Visual C++ Redistributable (a fresh Windows 10 or 11 install), and on an account whose global storage path makes the model's file paths longer than 260 characters. Then rename `bin/wake-word-engine` in the installed extension and enable listening: the error notification names that path.
...
- The runtime files set floors of their own: the macOS ONNX Runtime needs macOS 14.0. The Windows one imports the Visual C++ runtime (`msvcp140.dll`, `msvcp140_1.dll`, `vcruntime140.dll`, `vcruntime140_1.dll`), which CI runners have and a fresh Windows install may not, so `stage.mjs` ships those four beside the binary, where Windows resolves imports first, with `VC-RUNTIME-NOTICES.md`. They are unpacked, with Windows' own `expand.exe` and without running it, from Microsoft's redistributable installer, pinned by URL and digest in `C_RUNTIME` in `pinned-inputs.mjs`, and each library is pinned too; staging `win32-x64` therefore needs Windows. **NEVER** replace that list with an assumed one: `verify-vsix.mjs` reads the imports of every packaged library and fails when one names a Visual C++ runtime library that is not packaged, so a new ONNX Runtime that imports another fails the package. The libraries must be at least as new as the toolset that built `onnxruntime.dll` (its header's linker version, 14.44 today); bump the installer pin with the ONNX Runtime pin. No CI runner lacks the redistributable, so no CI check shows the engine starting without it. The Node engine loads ONNX Runtime from its own directory, where no runtime library ships.
